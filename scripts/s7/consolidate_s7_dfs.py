...
# from ppxftests.mockspec import FWHM_WIFES_INST_A # TODO fix broken import
...
_, _, metallicities, ages =\
    log_rebin_and_convolve_stellar_templates(isochrones="Padova", metals_to_use=['004', '008', '019'], 
...
for ap in [Aperture.FOURAS, Aperture.ONEKPC, Aperture.RE1]:
    ...
    for gg, gal in tqdm(enumerate(gals)):
        ...
        # Open the dataFrames
        df_ppxf = pd.read_hdf(os.path.join(s7_data_path, df_ppxf_fname)).set_index("Galaxy")
        df_stekin = pd.read_hdf(os.path.join(s7_data_path, df_stekin_fname)).set_index("Galaxy")
    ...
    df_all["Number"] = np.arange(len(gals))
    ...
    df_agn.to_csv(os.path.join(s7_data_path, f"s7_ppxf_{ap.name}_xAGN.csv"))
    ...
    df_av.to_csv(os.path.join(s7_data_path, f"s7_ppxf_{ap.name}_A_V.csv"))
    ...
    df_ages.to_csv(os.path.join(s7_data_path, f"s7_ppxf_{ap.name}_ages.csv"))

    # Only save kinematics
    cols = [c for c in df_all if "VdS" in c]
    df_stekin = df_all[cols].copy()
    df_stekin.to_csv(os.path.join(s7_data_path, f"s7_ppxf_{ap.name}_kinematics.csv"))
