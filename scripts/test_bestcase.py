...
if debug:
    ...
    df_metadata = pd.read_csv("../sim_galaxies/gal_metadata.csv")
    gals = [int(g) for g in df_metadata.index.values]

for gal in gals:
    # Filename 
    df_fname = f"ga{gal:004d}_bestcase.hd5" if not debug else f"ga{gal:004d}_DEBUG.hd5"
    ...
    for alpha_nu, x_AGN, A_V in product(alpha_nu_vals, x_AGN_vals, A_V_vals):
        ...
        try:
            ...
            print(f"ERROR: Generating spectrum failed for galaxy {gal} with A_V = {A_V}, x_AGN = {x_AGN} and alpha_nu = {alpha_nu}. Skipping...")
            continue
        ...
        df.to_hdf(os.path.join(data_path, df_fname), key="best_case")
