...
fit_agn_cont = False  # NOTE: run this TWICE - once with AGN cont. in the fit, once without.
...
alpha_nu_vals = [np.nan]
x_AGN_vals = [np.nan] # NOTE: NaNs are included to create a "special case" where we run without the AGN continuum
...
def add_stuff_to_df(pp_mc_list, pp_regul, ages, isochrones):
    ...
    if reddening is not None:
        thisrow["A_V (MC mean)"] = [3.1 * np.nanmean([pp.reddening for pp in pp_mc_list])]
        thisrow["A_V (MC error)"] = [3.1 * np.nanstd([pp.reddening for pp in pp_mc_list])]
        thisrow["A_V (regularised)"] = [pp_regul.reddening * 3.1]
...
for gal in gals:
    # Filename 
    if mpoly == -1:
        df_fname = f"ga{gal}_ext_agncontfit={'true' if fit_agn_cont else 'false'}_ext.hd5"
    else:
        df_fname = f"ga{gal}_ext_agncontfit={'true' if fit_agn_cont else 'false'}_mpoly.hd5"
    ...
    for alpha_nu, x_AGN, A_V in tqdm(product(alpha_nu_vals, x_AGN_vals, A_V_vals), total=(len(A_V_vals) * ((len(x_AGN_vals) - 1)*(len(alpha_nu_vals) - 1) + 1))):
        ...
        df.to_hdf(os.path.join(data_path, df_fname), key="agn")
